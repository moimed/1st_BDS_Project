...
namespace GestionVisite
{
    public partial class Form1 : Form
    {
        ...
        private void btnSaisirComm_Click(object sender, EventArgs e)
        {
            ...
            distanceLabel.Text = calculateDistanceParcourue().ToString();
        }
        ...
        private double calculateDistanceParcourue()
        {
           double distance = Helper.Distance(Commercial, ordre[0]);
            for (int index = 0; index <= ordre.Count() -2 ; index++)
            {
                distance += Helper.Distance(ordre[index], ordre[index + 1]);
            }
            return distance;
        }
    }
